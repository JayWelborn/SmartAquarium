...
class UserViewsetTests(APITestCase):
    ...
    def test_post_with_valid_data(self):
        ...
        user = get_user_model().objects.get(id=len(self.users) + 1)
        ...
        self.assertEqual(len(self.users) + 1, len(get_user_model().objects.all()))
    ...
    def test_put_with_valid_data(self):
        ...
        user = get_user_model().objects.get(id=self.users[0].id)
        pk = user.pk
        new_email = {
            'email': 'new@new.new'
        }
    ...
    def test_delete_with_staff(self):
        ...
        self.assertRaises(get_user_model().DoesNotExist, get_user_model().objects.get, pk=pk)
...
class UserProfileViewsetTests(APITestCase):
    ...
    def test_unauthenticated_user_can_only_get(self):
        ...
        for index, profile in enumerate(UserProfile.objects.all()):
            self.assertEqual(
                results[index]['created_date'],
                profile.created_date.isoformat().replace('+00:00', 'Z')
            )
        ...
        data = response.data
        self.assertEqual(response.status_code, 200)
        self.assertEqual(data['time_zone'], profile.time_zone)
    ...
    def test_wrong_user_can_only_get(self):
        ...
        response = self.detailview(request, pk=pk)
        self.assertEqual(response.status_code, 200)
        response_date = response.data['created_date']
        profile_date = profile.created_date.isoformat().replace('+00:00', 'Z')
        self.assertEqual(response_date, profile_date)
    ...
    def test_user_can_modify_and_delete_themselves(self):
        ...
        response = self.detailview(request, pk=pk)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['time_zone'], pytz.timezone(data['time_zone']))
        self.assertEqual(
            response.data['created_date'],
            profile.created_date.isoformat().replace('+00:00', 'Z')
        )

        # `DELETE` should work on self
